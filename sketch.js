...
for(var i=0;i<playerArrows.length;i++){
  var baseCollides=Matter.SAT.Collide(
    playerArrows[i].body,
    computerBase.body
  );
  var bodyCollides=Matter.SAT.collide(
    playerArrows[i].body,
    computerArcher.body
  );
  var cCollides=Matter.SAT.collide(
    playerArrows[i].body,
    computer.body
  );
  if(baseCollides.collided || bodyCollides.collided || cCollides.collided ){
...
function handleComputerArrowCollision() {
  ...
  for(var i=0;i<computerArrows.length;i++){
    var cbaseCollides=Matter.SAT.Collide(
      computerArrows[i].body,
      playerBase.body
    );
    var cbodyCollides=Matter.SAT.collide(
      computerArrows[i].body,
      playerArcher.body
    );
    var coCollides=Matter.SAT.collide(
      computerArrows[i].body,
      palyer.body
    );
    if(cbaseCollides.collided || cbodyCollides.collided || coCollides.collided ){
